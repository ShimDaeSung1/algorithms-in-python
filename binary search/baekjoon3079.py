...
# 가장 처음에 모든 심사대는 비어있고, 심사를 할 준비를 모두 끝냈다. 상근이와 친구들은 비행기 하나를 전세내고 놀러갔기 때문에, 
...
# 14초가 되었을 때는 다섯 번째 사람이 첫 번째 심사대로 이동해서 심사를 받으면 된다. 20초가 되었을 때, 두 번째 심사대가 비어있게 된다. 하지만, 
# 여섯 번째 사람이 그 곳으로 이동하지 않고, 1초를 더 기다린 다음에 첫 번째 심사대로 이동해서 심사를 받으면, 모든 사람이 심사를 받는데 걸리는 시간이 28초가 된다. 
...
#최단 시간 심사대 : 가장 짧은 심사대에서 한 명의 친구가 검사
left = min(times)
#최장 시간
right = max(times)*m
...
while left <= right:
    # total은 mid시간동안 심사 가능한 사람 수
    total = 0
    mid = (left+right)//2
    for i in range(len(times)):
        # 각 심사대별로 mid시간동안 심사할 수 있는 사람수 구한다.
        total += mid // times[i]
    # 최소 m명 이상 심사 가능하다면 mid를 줄여도 되므로 right가 mid가 됨. 
    if total >= m:
        answer = min(answer, mid)
        right = mid-1
    # 최소 m명 심사 불가능하다면 시간을 더 늘려야하므로(mid가 커져야함) 
    else:
        left = mid+1
print(answer)
